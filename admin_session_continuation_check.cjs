...
function copyRecursive(src, dest) {
  ...
  try {
    ...
    // Ignore files locked by the running browser; auth cookies often still copy successfully.
  }
}
...
async function detectDashboard(page) {
  ...
  const signals = await page.evaluate(() => {
    ...
    const selectors = ['aside', 'nav', '[role="navigation"]', '[class*="sidebar"]', '[class*="menu"]', '[class*="card"]', 'main'];
...
(async () => {
  ...
  for (const candidate of candidates) {
    ...
    if (outcome.dashboard && outcome.dashboard.looksLikeDashboard) {
      break;
    }
  }

  const success = summary.candidates.find((item) => item.dashboard && item.dashboard.looksLikeDashboard);
  summary.selected = success ? success.candidate : null;
